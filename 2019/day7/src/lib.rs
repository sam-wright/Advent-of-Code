...
mod tests {
    ...
    fn run(mut memory: &mut [isize], mut inputs: Vec<isize>) -> isize {
        ...
        let mut output = 0;
        output = read_program(&mut memory, &mut vec![output, inputs[0]]);
        output = read_program(&mut memory, &mut vec![output, inputs[1]]);
        output = read_program(&mut memory, &mut vec![output, inputs[2]]);
        output = read_program(&mut memory, &mut vec![output, inputs[3]]);
        output = read_program(&mut memory, &mut vec![output, inputs[4]]);
